...
class ICMPSocket:
    ...
    def _create_packet(self, request: ICMPRequest):
        ...
        code = struct.pack("B", 8)
        type = struct.pack("B", 0)
        id_bytes = struct.pack(">H", id)
        sequence_bytes = struct.pack(">H", sequence)
        raw_icmp_bytes = code + type + id_bytes + sequence_bytes + payload
        checksum = self._checksum(raw_icmp_bytes)
        checksum = struct.pack(">H", checksum)
        icmp_bytes = code + type + checksum + id_bytes + sequence_bytes + payload
        return icmp_bytes

    def _parse_reply(self, packet, source, current_time):
        check_sum = struct.unpack(">H", packet[self._ICMP_CHECKSUM_OFFSET: self._ICMP_ID_OFFSET])[0]
        if self._check_data(packet[self._ICMP_HEADER_OFFSET: self._ICMP_CHECKSUM_OFFSET]
                            + packet[self._ICMP_ID_OFFSET:], check_sum):
            type = struct.unpack(">B", packet[self._ICMP_HEADER_OFFSET: self._ICMP_CODE_OFFSET])[0]
            code = struct.unpack(">B", packet[self._ICMP_CODE_OFFSET: self._ICMP_CHECKSUM_OFFSET])[0]
            if code in [0, 8]:
                id = struct.unpack(">H", packet[self._ICMP_ID_OFFSET: self._ICMP_SEQUENCE_OFFSET])[0]
                sequence = struct.unpack(">H", packet[self._ICMP_SEQUENCE_OFFSET: self._ICMP_PAYLOAD_OFFSET])[0]
            else:
                id = struct.unpack(">H", packet[self._ICMP_PAYLOAD_OFFSET + self._ICMP_ID_OFFSET:
                            self._ICMP_PAYLOAD_OFFSET + self._ICMP_SEQUENCE_OFFSET])[0]
                sequence = struct.unpack(">H", packet[self._ICMP_PAYLOAD_OFFSET + self._ICMP_SEQUENCE_OFFSET:
                            self._ICMP_PAYLOAD_OFFSET + self._ICMP_PAYLOAD_OFFSET])[0]
        else:
            return None
        # TODO:
